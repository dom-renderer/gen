<script>
    ...
    function populateFormFromData(data, type) {
        if (type === 'PolicyHolder') {
            if (data.entity_type) {
                $(`input[name="type"][value="${ucwords(data.entity_type)}"]`).prop('checked', true);
                toggleEntityTypeFields();
            }

            if (data.full_name) $('#name').val(data.full_name);
            if (data.first_name) $('input[name="first_name"]').val(data.first_name);
            if (data.middle_name) $('input[name="middle_name"]').val(data.middle_name);
            if (data.last_name) $('input[name="last_name"]').val(data.last_name);
            if (data.place_of_birth) $('#place_of_birth').val(data.place_of_birth);
            if (data.date_of_birth) $('#dob').val(data.date_of_birth);
            if (data.country) $('#country').val(data.country).trigger('change');
            ...
            if (data.personal_status_other) $('input[name="entity_status_other"]').val(data.personal_status_other).removeClass('d-none');
            if (data.national_country_of_registration) $('select[name="national_country_of_registration"]').val(data.national_country_of_registration);
            if (data.gender) $(`input[name="gender"][value="${data.gender}"]`).prop('checked', true);
            if (data.country_of_legal_residence) $('select[name="country_of_legal_residence"]').val(data.country_of_legal_residence).trigger('change');
            if (data.passport_number) populatePassportData(data.passport_number, data.country_of_issuance);
            if (data.tin) populateTinData(data.tin);
            if (data.lei) $('input[name="lei"]').val(data.lei);
            if (data.phone_number) $('input[name="phone_number"]').val(data.phone_number);
            if (data.dial_code) $('input[name="dial_code"]').val(data.dial_code);
            ...
            if (data.last_name) $('input[name="last_name"]').val(data.last_name);
            if (data.place_of_birth) $('#place_of_birth').val(data.place_of_birth);
            if (data.date_of_birth) $('#dob-b-2').val(data.date_of_birth);
            if (data.country) $('#country').val(data.country).trigger('change');
            if (data.city) $('#city').val(data.city);
            if (data.zipcode) $('#zipcode').val(data.zipcode);
            if (data.address_line_1) $('#address_line_1').val(data.address_line_1);
            if (data.personal_status) $(`input[name="status"][value="${data.personal_status}"]`).prop('checked', true);
            if (data.smoker_status) $(`input[name="smoker_status"][value="${data.smoker_status}"]`).prop('checked', true);
            if (data.national_country_of_registration) $('input[name="national_country_of_registration"]').val(data.national_country_of_registration);
            if (data.gender) $(`input[name="gender"][value="${data.gender}"]`).prop('checked', true);
            if (data.country_of_legal_residence) $('select[name="country_of_legal_residence"]').val(data.country_of_legal_residence).trigger('change');
            if (data.passport_number) populatePassportDataB2(data.passport_number, data.country_of_issuance);
            if (data.notes) $('input[name="relationship_to_policyholder"]').val(data.notes);
            if (data.email) $('input[name="email"]').val(data.email);
            if (data.all_countries) populateTaxResidenceDataB2(data.all_countries);
        } else if (type === 'PolicyInsuredLifeInformation') {
            if (data.entity_type) $(`input[name="c1_type"][value="${ucwords(data.entity_type)}"]`).prop('checked', true);
            if (data.full_name) $('#c1_controlling_person_name').val(data.full_name);
            if (data.first_name) $('input[name="first_name"]').val(data.first_name);
            if (data.middle_name) $('input[name="middle_name"]').val(data.middle_name);
            if (data.last_name) $('input[name="last_name"]').val(data.last_name);
            if (data.place_of_birth) $('#c1_place_of_birth').val(data.place_of_birth);
            if (data.date_of_birth) $('#c1_date_of_birth').val(data.date_of_birth);
            if (data.address_line_1) $('#c1address').val(data.address_line_1);
            if (data.country) $('#c1country').val(data.country).trigger('change');
            if (data.city) $('#c1city').val(data.city);
            if (data.zipcode) $('#c1zip').val(data.zipcode);
            if (data.personal_status) $(`input[name="status"][value="${data.personal_status}"]`).prop('checked', true);
            if (data.smoker_status) $(`input[name="smoker_status"][value="${data.smoker_status}"]`).prop('checked', true);
            if (data.national_country_of_registration) $('#c1nationality').val(data.national_country_of_registration);
            if (data.gender) $(`input[name="gender"][value="${data.gender}"]`).prop('checked', true);
            if (data.country_of_legal_residence) $('#c1country_of_legal_residence').val(data.country_of_legal_residence).trigger('change');
            if (data.passport_number) populatePassportDataC1(data.passport_number, data.country_of_issuance);
            if (data.relationship_to_policyholder) $('#c1relationship_to_policyholder').val(data.relationship_to_policyholder);
            if (data.email) $('#c1email').val(data.email);
            if (data.all_countries) populateTaxResidenceDataC1(data.all_countries);

            $('#c-1-edit-id').val(data.id);
            toggleEntityTypeFieldsC1();
        } else if (type === 'PolicyBeneficiary') {
            if (data.entity_type) $(`input[name="d1_type"][value="${ucwords(data.entity_type)}"]`).prop('checked', true);
            if (data.full_name) $('input[name="full_name"]').val(data.full_name);
            if (data.first_name) $('input[name="first_name"]').val(data.first_name);
            if (data.middle_name) $('input[name="middle_name"]').val(data.middle_name);
            if (data.last_name) $('input[name="last_name"]').val(data.last_name);
            if (data.place_of_birth) $('#d-1-place-of-birth').val(data.place_of_birth);
            if (data.date_of_birth) $('#d-1-dob').val(data.date_of_birth);
            if (data.address_line_1) $('#d-1-address').val(data.address_line_1);
            if (data.country) $('#d-1-country').val(data.country).trigger('change');
            if (data.city) $('#d-1-city').val(data.city);
            if (data.zipcode) $('#d-1-zip').val(data.zipcode);
            if (data.personal_status) $(`input[name="d-1-status"][value="${data.personal_status}"]`).prop('checked', true);
            if (data.smoker_status) $(`input[name="d-1-smoker"][value="${data.smoker_status}"]`).prop('checked', true);
            if (data.nationality) $('#d-1-nationality').val(data.nationality);
            if (data.gender) $(`input[name="d-1-gender"][value="${data.gender}"]`).prop('checked', true);
            if (data.country_of_legal_residence) $('#d-1-legal-residence').val(data.country_of_legal_residence).trigger('change');
            if (data.passport_number) populatePassportDataD1(data.passport_number, data.country_of_issuance);
            if (data.relationship_to_policyholder) $('#d-1-relationship').val(data.relationship_to_policyholder);
            if (data.email) $('#d-1-email').val(data.email);
            if (data.dial_code) $('#d-1-dial_code').val(data.dial_code);
            if (data.phone_number) $('#d-1-phone_number').val(data.phone_number);
            if (data.beneficiary_death_benefit_allocation) $('#d-1-allocation').val(data.beneficiary_death_benefit_allocation);
            if (data.designation_of_beneficiary) $(`input[name="d-1-designation"][value="${data.designation_of_beneficiary}"]`).prop('checked', true);
