...
<form method="POST" action="{{ route('roles.update', encrypt($role->id)) }}">
    @csrf
    @method('PUT')
        <div class="col-md-12 pd-manage-del">
        <div class="card">
                    ...
                    @forelse($permissions as $key => $permission)
                        <div class="col-6">
                            <div class="card">
                                <div class="card-body">
                                    <h3>
                                        <input type="checkbox" class=" parent-checkbox check-main" id="{{ Str::slug($key) }}">
                                        <label for="{{ Str::slug($key) }}"> {{  ucwords(str_replace('-', ' ', $key))  }} </label>
                                    </h3>
                                    @forelse($permission as $row)
                                    <input type="checkbox" name="permissions[]" data-parent="{{ Str::slug($key) }}" class="check-main" id="{{ $row->name }}" value="{{ $row->id }}" @if(in_array($row->id, $existingPermissions)) checked @endif>
